...
test.serial('can write file', async t => {
    const success = await peer.assetSync.storagePlugin.writeFile('/testdir/test.txt', fileData)
    t.true(success)
})

test.serial('can read file', async t => {
    const data = await peer.assetSync.storagePlugin.readFile('/testdir/test.txt')
    t.assert(data, fileData)
})

test.serial('can remove file', async t => {
    const data = await peer.assetSync.storagePlugin.removeFile('/testdir/test.txt')
    t.true(data)
})

test.serial('can remove directory', async t => {
    const data = await peer.assetSync.storagePlugin.removeDirectory('/testdir')
    t.true(data)
})
