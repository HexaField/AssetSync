...
import { TextureLoader } from '../core/util/BitmapTextureLoader'
import _ from 'lodash'
import { createGeometry, createMaterial } from '../core/util/wireframe/index.js'
...
{  
    ...
    async createDefaultAssets()
    {
        ...
        this.missingTexture = await this.load.texture('missing_texture');
        // this.missingTextureData = this.createB64FromImage(this.missingTexture.image)
        
        this.mengerTexture = await this.load.texture('menger_texture');
        ...
        this.ponderTexture = await this.load.texture('ponder_texture');
        ...
        this.defaultTexture = await this.load.texture('default_texture');
        ...
        this.defaultMesh = (await this.load.gltf('sword')).scene;
    ...
    async createAsset(type, data, name)
    {
        ...
        switch(type)
        {
            case ASSET_TYPE.TEXTURE:  // this expects data to be in base 64
                ...
                if(exists)
                    return exists;
                let texture = await this.load.texture(data);
    ...
    async loadImageAssetFromHash(name)
    {
        ...
        //     asset = await this.getDataHandler(SERVER_PROTOCOLS.LOAD_ASSET, 'assets/' + name)
        // else
        //     asset = await this.getDataHandler(SERVER_PROTOCOLS.REQUEST_ASSET, 'assets/' + name)
    ...
    async saveAsset(type, path, data, name, metaData = {})
    {
        ...
        // if(this.enableCaching)
        //     await this.getDataHandler(SERVER_PROTOCOLS.SAVE_ASSET, { data: data, metaData:metaData })
    ...
    setByIPFSHash(type, hash, data, name, metaData = {})
    {
        ...
        // if(this.getScreens())
        // {
        //     this.getScreens().screenAssets.updateAssets();
        //     this.getScreens().screenAssetSelect.updateAssets();
        // }
        return this.assets[type][hash];
    }
