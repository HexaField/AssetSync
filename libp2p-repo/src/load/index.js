...
const { ERR_REPO_NOT_INITIALIZED } = require('../errors')
...
const createLibP2P = require('./libp2p')
   ...
   * @param {string || Libp2pRepo} options.repo repo location or instance to start
   * @param {Object} options.libp2pConfig
   */
const startRepo = async (options) => {
  ...
  return { peerId, keychain, repo, print, isNew }
}
module.exports = startRepo
...
const loadRepo = async (repo, options) => {
  ...
  if (openError == null) {
    ...
    if (options.allowNew === false) {
      throw new Error('Initialization of new repos disabled by config, pass `config.init.isNew: true` to enable it')
    } else {
...
const initRepo = async (repo, options) => {
  ...
  const libp2p = createLibP2P({
    ...
    libp2pConfig: options.libp2pConfig
  })
...
/**
 * Applies `config` to an open repo.
 *
 * @param {Repo} repo
 * @param {ConfigureOptions} options
 * @returns {Promise<{peerId: PeerId, keychain:Keychain}>}
 */
const configureRepo = async (repo, { libp2pConfig, pass }) => {
  const original = await repo.config.getAll()

  let peerId
  if(original.Identity) {
    peerId = await PeerId.createFromPrivKey(original.Identity.PrivKey)
  }
  const libp2p = createLibP2P({
